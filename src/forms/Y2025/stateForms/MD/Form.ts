...
export class MDForm extends StateFormBase {
  f1040: StateF1040
  state: State = 'MD'
  formName = '502'
  formOrder = 0
  ...
  stateStandardDeduction = (): number => {
    ...
    const pct = Math.round(agi * 0.15)
    const isJoint = this.filingStatus() === FilingStatus.MFJ || this.filingStatus() === FilingStatus.W
    const min = isJoint ? 3600 : 1800
    const max = isJoint ? 5150 : 2550
...
const makeMDForm = (f1040: StateF1040): MDForm => new MDForm(f1040)
export default makeMDForm
