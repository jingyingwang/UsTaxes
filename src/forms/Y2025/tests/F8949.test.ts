...
const makeAsset = (overrides: Partial<Asset<Date>> = {}): SoldAsset<Date> => ({
  ...
  openDate: new Date(CURRENT_YEAR, 0, 15),
  closeDate: new Date(CURRENT_YEAR, 6, 15),
  openPrice: 100,
  closePrice: 150,
  openFee: 0,
  closeFee: 0,
...
describe('F8949', () => {
  describe('box routing', () => {
    it('should assign short-term assets to Box C (unreported)', () => {
    ...
    it('should assign long-term assets to Box F (unreported)', () => {
      const asset = makeAsset({
        openDate: new Date(2023, 0, 15),
        closeDate: new Date(CURRENT_YEAR, 6, 15)
      })
      const f1040 = new F1040(makeInfo(), [asset])
      const f8949 = new F8949(f1040)
  ...
  describe('holding period classification', () => {
    it('should classify <= 366 days as short-term', () => {
      const asset = makeAsset({
        openDate: new Date(CURRENT_YEAR, 0, 1),
        closeDate: new Date(CURRENT_YEAR, 11, 31)
      })
      const f1040 = new F1040(makeInfo(), [asset])
      const f8949 = new F8949(f1040)
    ...
    it('should classify > 366 days as long-term', () => {
      const asset = makeAsset({
        ...
        closeDate: new Date(CURRENT_YEAR, 1, 2)
      })
      const f1040 = new F1040(makeInfo(), [asset])
      const f8949 = new F8949(f1040)
  ...
  describe('pagination (copies)', () => {
    it('should create additional copies for overflow transactions', () => {
      ...
      const f1040 = new F1040(makeInfo(), assets)
      const f8949 = new F8949(f1040)
      const copies = f8949.copies()
