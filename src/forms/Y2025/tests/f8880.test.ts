...
describe("Form 8880 - Saver's Credit", () => {
  ...
  it('should handle MFJ with both spouses contributing', () => {
    ...
    // AGI = 38000 for MFJ => 50% rate (threshold 46000)
    expect(f8880.l9()).toEqual(0.5)
  ...
  it('should be included in schedules when needed', () => {
    const information = cloneDeep(baseInformation)
    const f1040 = new F1040(information, [])
    const f8880 = new F8880(f1040)
    ...
    if (f8880.isNeeded()) {
      ...
      const hasF8880 = schedules.some((s) => s.tag === 'f8880')
      expect(hasF8880).toBe(true)
    }
  })
