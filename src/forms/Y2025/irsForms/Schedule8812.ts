...
// ITINs (Individual Taxpayer Identification Numbers) always start with "9".
// Children must have a valid SSN (not ITIN) to qualify for the full CTC.
// Children with ITINs only qualify for the $500 Other Dependent Credit.
const hasSSN = (person: Person): boolean => {
  const digits = person.ssid.replace(/\D/g, '')
  return digits.length > 0 && digits[0] !== '9'
}
...
export default class Schedule8812 extends F1040Attachment {
  ...
  // Only qualifying children with SSN (not ITIN) qualify for the full CTC.
  // Children with ITINs are counted as "other dependents" on line 6.
  creditDependents = (): Dependent[] =>
    this.f1040.qualifyingDependents
      .qualifyingChildren()
      .filter((dep) => hasSSN(dep))

  l4 = (): number => this.creditDependents().length

  // TY2025 OBBBA: $2,200 per qualifying child (up from $2,000)
  ...
  part2a = (): Part2a => {
    ...
    // TY2025 OBBBA: $1,700 refundable per qualifying child (up from $1,600)
    ...
    const l20 = l19 * 0.15
    // TY2025: threshold is 3 × $1,700 = $5,100 (was 3 × $1,600 = $4,800)
