...
describe('CSV import parsers', () => {
  test('Fidelity parser extracts 1099 summary data', () => {
    ...
    const f1099b = f1099s.find((f) => f.type === Income1099Type.B)
    const f1099div = f1099s.find((f) => f.type === Income1099Type.DIV)
    const f1099int = f1099s.find((f) => f.type === Income1099Type.INT)
    if (!f1099b || !f1099div || !f1099int) {
      throw new Error('Expected 1099-B, 1099-DIV, and 1099-INT forms')
    }
    expect(f1099b.form.shortTermProceeds).toBeCloseTo(1000)
    expect(f1099b.form.shortTermCostBasis).toBeCloseTo(700)
    expect(f1099b.form.longTermProceeds).toBeCloseTo(2000)
    expect(f1099b.form.longTermCostBasis).toBeCloseTo(1500)
  ...
  test('Generic parser handles standard 1099 columns', () => {
    ...
    expect(f1099b.form.shortTermProceeds).toBeCloseTo(300)
    expect(f1099b.form.longTermProceeds).toBeCloseTo(600)
  })
})
