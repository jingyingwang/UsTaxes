...
const formReducer = (
  ...
  switch (action.type) {
    ...
    case ActionName.ADD_F3922: {
      return {
        ...newState,
        f3922s: [...newState.f3922s, action.formData]
      }
    }
    case ActionName.EDIT_F3922: {
      const newf3922s = [...newState.f3922s]
      newf3922s.splice(action.formData.index, 1, action.formData.value)
      return {
        ...newState,
        f3922s: newf3922s
      }
    }
    case ActionName.REMOVE_F3922: {
      const newf3922s = [...newState.f3922s]
      newf3922s.splice(action.formData, 1)
      return {
        ...newState,
        f3922s: newf3922s
      }
    ...
    case ActionName.ADD_SCHEDULE_C: {
      return {
        ...newState,
        scheduleCInputs: [...newState.scheduleCInputs, action.formData]
      }
    }
    case ActionName.EDIT_SCHEDULE_C: {
      const newScheduleCs = [...newState.scheduleCInputs]
    ...
    case ActionName.ADD_SCHEDULE_F: {
      return {
        ...newState,
        scheduleFInputs: [...newState.scheduleFInputs, action.formData]
      }
    }
    case ActionName.EDIT_SCHEDULE_F: {
      const newScheduleFs = [...newState.scheduleFInputs]
      newScheduleFs.splice(action.formData.index, 1, action.formData.value)
      return {
        ...newState,
        scheduleFInputs: newScheduleFs
      }
    }
    case ActionName.REMOVE_SCHEDULE_F: {
      const newScheduleFs = [...newState.scheduleFInputs]
      newScheduleFs.splice(action.formData, 1)
      return {
        ...newState,
        scheduleFInputs: newScheduleFs
      }
...
> = combineReducers({
  ...
  activeYear
}) as unknown as Reducer<CombinedState<YearsTaxesState>, Actions>
